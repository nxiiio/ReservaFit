...
- `app/lib/api.ts` owns the API calls (axios instance): `acquireApiToken` (`acquireTokenSilent`, with redirect fallback on `InteractionRequiredAuthError` or `timed_out`) and the backend URL (`http://localhost:8080`, hardcoded). `app/lib/use-current-user.ts` calls `POST /api/usuarios/me` and returns the profile.
